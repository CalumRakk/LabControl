...
def wait_for_lab_load(driver: Chrome, timeout: int = 30):
    try:
        ...
        with switch_to_iframe(driver, timeout):
            ...
            logger.info(f"¡Contenido del iframe cargado! Elemento encontrado: {elemento_dentro.get_attribute('onclick')}")
            return True
...
def get_lab_aws_details(driver: Chrome)->list[str]:
    with switch_to_iframe(driver):
        ...
        wait = WebDriverWait(driver, 10)
        aws_details_content= wait.until(EC.presence_of_element_located(("id", "awsdetailsframe")))
