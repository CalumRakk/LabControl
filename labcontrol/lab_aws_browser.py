...
from labcontrol.browser.actions_lab_aws import (
    get_course_id,
    get_lab_aws_details,
    get_lab_item_id,
...
from labcontrol.vocareum_http import VocareumParams
...
class LabAWSBrowserAPI:
    ...
    def __init__(
        self,
        cookies: Optional[List[SeleniumCookie]] = None,
        chrome_profile: Optional[Union[str, Path]] = None,
        headless=False,
    ):
    ...
    def get_cookies_vocareum(self) -> List[SeleniumCookie]:
        with switch_to_iframe(self.browser.driver, 10):
            return [SeleniumCookie(**i) for i in self.browser.driver.get_cookies()]

    def set_and_is_valid_cookie(self, cookies: List[SeleniumCookie]) -> bool:
        """Verifica si las cookies proporcionadas son válidas para acceder al laboratorio."""
        try:
            url = "https://awsacademy.instructure.com/profile/settings"
            self._set_cookies(cookies)
            self.browser.driver.get(url)
            wait = WebDriverWait(self.browser.driver, 3)
            full_name_element = wait.until(
                EC.presence_of_element_located(
                    ("xpath", ".//span[@class='full_name display_data']")
                )
            )
            return full_name_element is not None
        except Exception as e:
            logger.error("Error al validar cookies: %s", e)
            return False

    def get_vocareum_params(self) -> VocareumParams:
        with switch_to_iframe(self.browser.driver, 10):
